...
export default function NoticeEditPage() {
  ...
  const {
    ...
    defaultValues: { extra_parameters: { conditions: [], severity: "" } },
    refineCoreProps: { meta: { select: "*" } },
  });

  // When the record is fetched, reset form values.
  useEffect(() => {
    // refine automatically resets when query result changes.
    // If needed, call reset(record) here.
  }, [reset]);

  // Options for incident conditions and severity.
  const conditionOptions = ["Weather", "Mechanical", "Operational", "Other"];
  const severityOptions = ["Low", "Medium", "High"];

  return (
    <Edit title="Edit Notice" saveButtonProps={saveButtonProps}>
      <Box
        component="form"
